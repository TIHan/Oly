...
let private filterFunctionsForOverloadingByWeight resArgs (returnTyOpt: TypeSymbol option) (funcs: IFunctionSymbol imarray) =
    ...
    let rec computeWeight (currentRigidWeight, currentWeight) (expectedTy: TypeSymbol) (ty: TypeSymbol) : struct(int * int) =
        ...
        if (ty.IsSolved || ty.IsObjectOrNumberInferenceVariable) && UnifyTypes Generalizable expectedTy ty then
...
let private filterFunctionsForOverloadingFinalPhase (funcs: IFunctionSymbol imarray) =
    ...
    if scoredResult.Length > 1 then
        let filteredFuncs = filterFunctionsForOverloadingByLeastGenericReturnType scoredResult
