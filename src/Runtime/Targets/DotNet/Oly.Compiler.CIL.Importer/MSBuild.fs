            ...
            try
                ...
                try
                    let msbuildTask = backgroundTask {
                        ...
                        use p = new ExternalProcess("dotnet", $"{build} -v minimal --disable-build-servers -c {configName} -o \"{outputPath.ToString()}\" \"{projectName}.csproj\"", workingDirectory = stubDir.FullName)
